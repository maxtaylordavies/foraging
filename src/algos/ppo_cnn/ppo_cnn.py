...
from src.algos.networks import CNN
...
def _make_policy(algo, ts):
    def act(rng, obs, state, extra):
        ...
        feats = algo.cnn.apply(ts.cnn_ts.params, obs)

        action = algo.actor.apply(ts.actor_ts.params, feats, rng, method="act")
        return jnp.squeeze(action), extra

        # action = jax.random.randint(
        #     rng, (1,), 0, algo.env.action_space(algo.env_params).n
        # )
        # return action.squeeze(), extra
...
class PPO_CNN(OnPolicyMixin, NormalizeObservationsMixin, Algorithm):
    ...
    def create(cls, **config):
        ...
        def eval_callback(algo, ts, rng):
            ...
            food, poison = metrics["food_consumed"], metrics["poison_consumed"]
            jax.debug.print(
                "iter: {}, mean return: {}, mean length: {}, mean food: {}, mean poison: {}",
                ts.global_step,
                returns.mean(),
                lengths.mean(),
                food.mean(),
            ...
            return returns, lengths, food, poison
    ...
    def create_agent(cls, config, env, env_params):
        ...
        return {"actor": actor, "critic": critic, "cnn": cnn}

    @register_init
    def initialize_network_params(self, rng):
        rng, rng_cnn, rng_actor, rng_critic = jax.random.split(rng, 4)
        init_obs = jnp.empty([1, *self.env.observation_space(self.env_params).shape])
        init_cnn_out = jnp.empty([1, 32])

        cnn_params = self.cnn.init(rng_cnn, init_obs)
        actor_params = self.actor.init(rng_actor, init_cnn_out, rng_actor)
        critic_params = self.critic.init(rng_critic, init_cnn_out)
        ...
        return {"actor_ts": actor_ts, "critic_ts": critic_ts, "cnn_ts": cnn_ts}

    def train_iteration(self, ts):
        ts, trajectories = self.collect_trajectories(ts)
        last_feats = self.cnn.apply(ts.cnn_ts.params, ts.last_obs)
    ...
    def collect_trajectories(self, ts):
        def env_step(ts, unused):
            ...
            # Sample action
            feats = self.cnn.apply(ts.cnn_ts.params, ts.last_obs)
    ...
    def update_actor(self, ts, batch):
        def loss_fn(cnn_params, actor_params):
            feats = self.cnn.apply(cnn_params, batch.trajectories.obs)
        ...
        cnn_grads, actor_grads = jax.grad(loss_fn, argnums=(0, 1))(
            ts.cnn_ts.params, ts.actor_ts.params
        )
    ...
    def update_critic(self, ts, batch):
        def loss_fn(cnn_params, critic_params):
            feats = self.cnn.apply(cnn_params, batch.trajectories.obs)
        ...
        cnn_grads, critic_grads = jax.grad(loss_fn, argnums=(0, 1))(
            ts.cnn_ts.params, ts.critic_ts.params
        )
...
def train_ppo_cnn(
    ...
    ep_returns, ep_lengths, food_counts, poison_counts = evaluation
    mean_returns, mean_lengths, mean_food, mean_poison = (
        ep_returns.mean(axis=1),
        ep_lengths.mean(axis=1),
        food_counts.mean(axis=1),
        poison_counts.mean(axis=1),
    )
    train_df = pd.DataFrame(
        {
            ...
            "return": mean_returns,
            "length": mean_lengths,
            "food_consumed": mean_food,
            "poison_consumed": mean_poison,
        }
    )
